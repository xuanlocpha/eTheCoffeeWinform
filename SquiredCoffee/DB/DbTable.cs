...
namespace SquiredCoffee.DB
{
    class DbTable
    {
        ...
        public static void AddTable(Table std)
        {
            string sql = "INSERT INTO tables (title,status) VALUES(@title,@status)";
        ...
        {
            string sql = "select * from tables where title = '" + @title + "'";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
        ...
        public static bool CheckStatusTable(string @id)
        {
            string sql = "select * from tables where id = '" + @id + "' AND status = 0";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
            MySqlDataAdapter adp = new MySqlDataAdapter(cmd);
            DataSet tbl = new DataSet();
        ...
        public static void UpdateTable(Table std, string id)
        {
            string sql = "UPDATE tables SET title = @title,status=@status  WHERE id = @id";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
            cmd.CommandType = CommandType.Text;
        ...
        public static bool CheckStatusDb(string @id)
        {
            string sql = "select * from tables where id = '" + @id + "' AND status ='"+0+"'";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
        ...
        public static void UpdateStatusTable(Table std, string id)
        {
            string sql = "UPDATE tables SET title = @title,status=@status  WHERE id = @id";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
        ...
        public static List<Table> LoadTableList()
        {
            ...
            DataTable data = DataProvider.Instance.ExecuteQuery("SELECT id,title,status FROM tables ");
        ...
        public static List<Table> LoadTableListDrum()
        {
            List<Table> tableList = new List<Table>();

            DataTable data = DataProvider.Instance.ExecuteQuery("SELECT id,title,status FROM tables WHERE status = '"+1+"'");
