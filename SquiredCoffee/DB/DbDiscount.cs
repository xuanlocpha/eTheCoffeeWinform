...
namespace SquiredCoffee.DB
{
    class DbDiscount
    {
        ...
        public static List<Discount> LoadDiscountList()
        {
            ...
            DataTable data = DataProvider.Instance.ExecuteQuery("SELECT d.id,d.discount,d.product_id,d.start_date,d.expiry_date,d.status,p.title as title_product FROM discounts d, products p WHERE d.product_id = p.id");
